...
*Setup instructions for a gitian build of FDM using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the FDM
...
guide. It is preferred to follow these stFDM yourself instead of using someone else's
...
At the end the VM is rebooted to make sure that the changes take effect. The stFDM in this
...
The rest of the stFDM in this guide will be performed as that user.
...
Clone the git repositories for FDM and gitian and then checkout the FDM version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/FDM-crypto/FDM.git
cd FDM
...
Follow the instructions in [doc/release-process.md](release-process.md) in the FDM repository
...
To build FDM (for Linux, OSX and Windows) just follow the stFDM under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the FDM repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/FDM/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/FDM-crypto/FDM
...
For example:
```bash
URL=https://github.com/FDMadnetwork/FDM.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit FDM=${COMMIT} --url FDM=${URL} ../FDM/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit FDM=${COMMIT} --url FDM=${URL} ../FDM/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit FDM=${COMMIT} --url FDM=${URL} ../FDM/contrib/gitian-descriptors/gitian-osx.yml
```
...
and follow the stFDM in the build process as normal.

    gpg: skipped "FDMcoin": secret key not available

When you execute `gsign` you will get an error from GPG, which can be ignored. Copy the resulting `.assert` files
in `gitian.sigs` to your signing machine and do

```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/FDM-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/FDM-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/FDM-build.assert
```
...
[FDM/gitian.sigs](https://github.com/FDM-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.

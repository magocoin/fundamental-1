...
#if defined(NDEBUG)
#error "FDM cannot be compiled without assertions."
#endif
...
{
    return; // This keFDM banning nodes, for no logical reason.
    if (howmuch == 0)
        return;
...
/**
 * Make the best chain active, in multiple stFDM. The result is either failure
...
int static inline GetSkipHeight(int height)
{
    ...
    // but the following expression seems to perform well in simulations (max 110 stFDM to go back
    // up to 2**18 blocks).

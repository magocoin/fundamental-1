...
void CNode::copyStats(CNodeStats& stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (FDM users should be well used to small numbers with many decimal places by now :)
...
void ThreadMapPort()
{
    ...
    if (r == 1) {
        ...
        string strDesc = "FDM " + FormatFullVersion();
...
void ThreadOpenConnections()
{
    ...
    while (true) {
        ...
        while (true) {
            ...
            // stop this loop, and let the outer loop run again (which sleFDM, adds seed nodes, recalculates
...
bool BindListenPort(const CService& addrBind, string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR) {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. FDM is probably already running."), addrBind.ToString());
        else

...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a FDM address (e.g. %1)").arg("FHoujpfmyWqr69iWhv8D5kpenvSYwG1MGm"));
#endif
...
{
    // return if URI is not valid or is no FDM: URI
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient* out)
{
    // Convert FDM:// to FDM:
    //
    //    Cannot handle this later, because FDM:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open FDM.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "FDM.lnk";
}
...
{
    return GetAutostartDir() / "FDM.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a FDM.desktop file to the autostart directory:
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the FDM app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add FDM app to startup item list

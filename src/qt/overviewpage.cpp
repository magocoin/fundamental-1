...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeFDMAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeFDMAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeFDMAmount = strAnonymizeFDMAmount.remove(strAnonymizeFDMAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeFDMAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeFDMAmount * COIN) nMaxToAnonymize = nAnonymizeFDMAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeFDMAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeFDMAmount));
        strAnonymizeFDMAmount = strAnonymizeFDMAmount.remove(strAnonymizeFDMAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeFDMAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {

        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true) {
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-FDM-address
                    CScript scriptChange;
...
string CWallet::PrepareObfuscationDenominate(int minRounds, int maxRounds)
{
    ...
    */
    ...
    int nStep = 0;
    int nStFDMMax = 5 + GetRandInt(5);
    while (nStep < nStFDMMax) {
        BOOST_FOREACH (CAmount v, obfuScationDenominations) {
...
bool CWallet::IsMine(const string& FDM_address)
{
    CBitcoinAddress check_address(FDM_address);

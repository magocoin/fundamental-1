...
// The COrphan class keFDM track of these 'temporary orphans' while
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("FDMMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("FDMMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("FDMMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("FDM-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running FDMMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),

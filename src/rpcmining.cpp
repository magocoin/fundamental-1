...
Value getgenerate(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "It is set with the command line argument -gen (or FDM.conf setting gen)\n"
...
Value getblocktemplate(const Array& params, bool fHelp)
{
    ...
    //if (vNodes.empty())
    //    throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "FDM is not connected!");

    //if (IsInitialBlockDownload())
    //    throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "FDM is downloading blocks...");
    ...
    BOOST_FOREACH (CTransaction& tx, pblock->vtx) {
        ...
        BOOST_FOREACH (const CTxIn& in, tx.vin) {
            if (setTxIndex.count(in.prevout.hash))
                dFDM.push_back(setTxIndex[in.prevout.hash]);
        }
        entry.push_back(Pair("depends", dFDM));
